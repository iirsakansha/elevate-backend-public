        ...
        try:
            ...
            if user.is_superuser and not hasattr(user, 'profile'):
                ...
                if user.profile.invitation_status.lower() == 'pending':
                    return Response({"error": "Invited users cannot request password reset"}, status=status.HTTP_403_FORBIDDEN)
                if user.profile.role.lower() not in ALLOWED_PASSWORD_RESET_ROLES:
                    return Response({"error": "Only admin users can request password reset"}, status=status.HTTP_403_FORBIDDEN)
            token = get_random_string(length=32)
            ...
            html_message = render_to_string(
                'emails/password-reset.html', context)
...
class PasswordResetLinkValidateAPI(APIView):
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            return Response({"valid": False, "error": "User is not allowed to reset password"}, status=status.HTTP_403_FORBIDDEN)
        except User.DoesNotExist:
            return Response({"valid": False, "error": "User does not exist"}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.error(f"Unexpected error in link validation: {str(e)}")
            return Response({"valid": False, "error": f"Unexpected error: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class AnalysisView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        except (InvalidFileError, InvalidCategoryError, InvalidDateError, InvalidTimeFormatError) as e:
            logger.error(f"Analysis failed: {str(e)}")
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except AnalysisProcessingError as e:
            logger.error(f"Analysis processing error: {str(e)}")
            return Response({'error': str(e), 'traceback': e.traceback}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
