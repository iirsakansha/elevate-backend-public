        ...
        try:
            with transaction.atomic():
                user = User.objects.create_user(
                    username=username, email=email, password=random_password, is_staff=False)
                UserProfile.objects.create(
                    user=user, organization=organization, invitation_status='Pending',
                    invitation_username=username, temporary_password=random_password, invitation_token=token, role='user')
        ...
        try:
            ...
            if user.is_superuser and not hasattr(user, 'profile'):
                ...
                if not hasattr(user, 'profile'):
                    return Response({"error": "User profile is missing"}, status=status.HTTP_400_BAD_REQUEST)

                profile = user.profile

                if profile.invitation_status.lower() == 'pending':
                    return Response(
                        {"error": "Invited users cannot request password reset"},
                        status=status.HTTP_403_FORBIDDEN
                    )

                if profile.role.lower() not in ALLOWED_PASSWORD_RESET_ROLES:
                    return Response(
...
class PasswordResetAPI(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # ✅ Case 1: Superuser without profile
            if user.is_superuser and not hasattr(user, "profile"):
                user.set_password(new_password)
                user.save()
                return Response({"message": "Password reset successfully"}, status=status.HTTP_200_OK)

            # ✅ Case 2: Admin user with profile and matching token
...
class PasswordResetLinkValidateAPI(APIView):
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            # ✅ Case 2: User with profile and matching token
            if hasattr(user, "profile"):
            ...
            # ❌ Case 3: No profile, and not a superuser
            return Response({
                "valid": False,
                "error": "User is not allowed to reset password."
            }, status=status.HTTP_403_FORBIDDEN)
...
class AnalysisView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            logger.info(
                f"Starting EV analysis for user {request.user.username} at {start_time}")
            ...
            if 'name' not in all_data:
                ...
                logger.info(
                    f"Set default name to {request.user.username} for invited user")
            ...
            serializer = AnalysisSerializer(
                data=all_data, context={'request': request})
            if not serializer.is_valid():
            ...
            self.cleanup_temporary_data(
                created_load_categories, created_vehicle_categories, None, file_path)
    ...
    def process_categories(self, all_data):
        ...
        for idx, category_data in enumerate(load_categories[:6], start=1):
            ...
            try:
                ...
                logger.error(
                    f"Load category {idx} ({category_data.get('category', 'unknown')}) validation failed: {str(e)}")
                raise ValueError(
                    f"Load category {idx} ({category_data.get('category', 'unknown')}) validation failed: {str(e)}")
        ...
        for idx, vehicle_data in enumerate(vehicle_categories[:5], start=1):
            ...
            try:
                ...
                logger.error(
                    f"Vehicle category {idx} ({vehicle_data.get('vehicleCategory', 'unknown')}) validation failed: {str(e)}")
                raise ValueError(
                    f"Vehicle category {idx} ({vehicle_data.get('vehicleCategory', 'unknown')}) validation failed: {str(e)}")
    ...
    def prepare_analysis_data(self, ev_instance):
        ...
        analysis_data = {
            ...
            'sharedSavaing': ev_instance.shared_saving,
    ...
    def run_full_analysis(self, input_data, folder_id):
        def load_forecast(n, f, c, p, e, r, k, l, g, h, s, u, rowlimit_xl, CAGR_V, tariff):
            ...
            Charging_Duration = (
                (60*c/input_data['resolution'])/(p*e/100)) * (EndingSOC_Matrix-StartingSOC_Matrix)
        ...
        for vehicle in input_data['vehicleCategoryData']:
            try:
                ...
                logger.error(
                    f"Load forecast failed for vehicle {vehicle.get('vehicleCategory', 'unknown')}: {str(e)}")
                raise
        ...
        try:
            ...
            Source['datetime_utc'] = pd.to_datetime(
                Source['datetime_utc'], errors='coerce')
        ...
        value_to_repeat = excelimport.iloc[0,
        ...
        Transformer_capacity['100% Transformer rated capacity'] = np.repeat(
            excelimport.iloc[0, 1], number_of_repeats)
        ...
        for year in range(1, 6):
            ...
            ev_loads.append({f'Year {year}': ddf_next.sum(
        ...
        for i, date in enumerate(dates):
            base_pattern = np.array(
                [60 + 40 * np.sin(2 * np.pi * (h) / 24) + np.random.normal(0, 5) for h in np.linspace(0, 24, 48)])
            if date.weekday() < 5:
        ...
        required_net_loads = [pd.DataFrame(
            selected_ranges[year]) + shazam_years[year].values for year in range(5)]
        combined_loads = []
        for year in range(5):
            ...
            combined_loads.append(
                {f'Year {year+1}': {'base_load': base_load, 'ev_load': ev_load}})
        ...
        overshots = [rn - (excelimport.iloc[0, 1] * (float(input_data['BR_F'])/100))
        ...
        for year in range(1, 6):
            ...
            overshot_data.append({f'Year {year}': {'planning': ov_p.to_dict(
        ...
        def calc_TOD_x(year_num):
            try:
                ...
                if denominator == 0:
                    logger.warning(
                        f"Zero denominator in calc_TOD_x for year {year_num}, returning 0")
                    return 0
                ...
                logger.error(
                    f"Error in calc_TOD_x for year {year_num}: {str(e)}")
                return 0
