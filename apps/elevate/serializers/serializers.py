...
class LoadCategoryModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "category", "category_file", "sales_cagr", "specify_split"]

    def validate(self, data):
        """Validate load category data."""
        if data.get("sales_cagr", 0) < 0:
            raise serializers.ValidationError("sales_cagr must be non-negative")
        if data.get("specify_split", 0) < 0:
            raise serializers.ValidationError("specify_split must be non-negative")
        return data
...
class VehicleCategoryModelSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if "handling_cost" in data and data["handling_cost"] > 100:
            raise serializers.ValidationError("handling_cost must be between 0 and 100")
        if "cagr_v" in data and data["cagr_v"] > 100:
            raise serializers.ValidationError("cagr_v must be between 0 and 100")
        return data
...
class PermanentAnalysisSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        total_split = sum(float(item.get("specify_split", 0)) for item in category_data)
        ...
        if "shared_saving" in data and float(data["shared_saving"]) > 100:
            raise serializers.ValidationError("shared_saving must be between 0 and 100")
        if "summer_sx" in data and float(data["summer_sx"]) > 100:
            raise serializers.ValidationError("summer_sx must be between 0 and 100")
        if "summer_rb" in data and float(data["summer_rb"]) > 100:
            raise serializers.ValidationError("summer_rb must be between 0 and 100")
        if "winter_sx" in data and float(data["winter_sx"]) > 100:
            raise serializers.ValidationError("winter_sx must be between 0 and 100")
        if "winter_rb" in data and float(data["winter_rb"]) > 100:
            raise serializers.ValidationError("winter_rb must be between 0 and 100")

        for date_field in ["summer_date", "winter_date"]:
            if date_field in data and not isinstance(data[date_field], list):
                raise serializers.ValidationError(f"{date_field} must be a list")
...
class AnalysisSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        total_split = sum(item.get("specify_split", 0) for item in category_data)
