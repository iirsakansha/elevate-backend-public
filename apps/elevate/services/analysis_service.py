...
from ..models.models import LoadCategoryModel, UserAnalysis, VehicleCategoryModel
from ..serializers.serializers import (
    AnalysisSerializer,
    LoadCategoryModelSerializer,
...
class AnalysisService:
    ...
    def process_analysis(self):
        ...
        try:
            ...
            all_data["is_load_split_file"] = (
                file_path
                if file_path
                else all_data.get("is_load_split_file", all_data.get("load_split_file"))
            )

            if "name" not in all_data:
                all_data["name"] = self.user.username

            processed_data, created_load_categories, created_vehicle_categories = (
                self._process_categories(all_data)
            )
    ...
    def _handle_file_upload(self, all_data, files):
        ...
        file_key = (
            "is_load_split_file"
            if "is_load_split_file" in all_data
            else "load_split_file"
        )
        ...
        if file_key in all_data and isinstance(all_data[file_key], str):
            ...
            os.makedirs(
                os.path.join(settings.MEDIA_ROOT, self.UPLOAD_FOLDER), exist_ok=True
            )
    ...
    def _process_categories(self, all_data):
        ...
        created_load_categories = []
        created_vehicle_categories = []
        load_categories = all_data.get("category_data", []) or all_data.get(
            "categoryData", []
        )
        ...
        vehicle_categories = all_data.get("vehicle_category_data", []) or all_data.get(
            "vehicleCategoryData", []
        )
    ...
    def _cleanup_temporary_data(
        ...
        with transaction.atomic():
            try:
                if load_categories:
                    LoadCategoryModel.objects.filter(
                        id__in=[cat.id for cat in load_categories]
                    ).delete()
            except Exception as e:
            ...
            try:
                if vehicle_categories:
                    VehicleCategoryModel.objects.filter(
                        id__in=[veh.id for veh in vehicle_categories]
                    ).delete()
            except Exception as e:
            ...
            try:
                ...
                logger.error(
                    f"Error in file cleanup for {load_split_file_url}: {e}",
                    exc_info=True,
                )
    ...
    def _calculate_tod_duration(self, pks, pke, sx, ops, ope, rb):
        ...
        try:
            ...
            return {
                ...
                "peak_end": pke,
                "peak_hours": (
                    datetime.combine(datetime.today(), pke_time)
                    - datetime.combine(datetime.today(), pks_time)
                ).seconds
                / 3600,
                "offpeak_start": ops,
                "offpeak_end": ope,
                "offpeak_hours": (
                    datetime.combine(datetime.today(), ope_time)
                    - datetime.combine(datetime.today(), ops_time)
                ).seconds
                / 3600,
            }
    ...
    def _run_full_analysis(self, input_data, folder_id):
        ...
        def load_forecast(
            ...
            try:
                ...
                blocks = np.arange(
                    1, int(1440 / input_data["resolution"]) + 1, 1
                ).reshape((1, int(1440 / input_data["resolution"])))
                ...
                ending_soc_matrix = np.tile(
                    ending_soc,
                    ((int(1440 / input_data["resolution"])) * (range_km + 1), 1),
                )
                ...
                charging_duration = (
                    ...
                    / (penetration_rate * energy_consumption)
                ...
                blo_load_sec = (penetration_rate * blo_sum_linear).reshape(
                    1, int(1440 / input_data["resolution"])
                )
        ...
        try:
            ...
            if (
                "datetime_utc" not in source_data.columns
                or source_data["datetime_utc"].isna().all()
            ):
                source_data["datetime_utc"] = pd.date_range(
                    start="2023-01-01",
                    periods=len(source_data),
                    freq=f'{input_data["resolution"]}min',
                )
            else:
        ...
        value_to_repeat = transformer_capacity_value * (float(input_data["br_f"]) / 100)
        ...
        if total_data_points % time_blocks_per_day != 0:
            load_data_trimmed = load_data.iloc[: complete_days * time_blocks_per_day]
        else:
        ...
        try:
            load_extract = pd.DataFrame(
                np.reshape(
                    load_data_trimmed.to_numpy(), (complete_days, time_blocks_per_day)
                )
            ).T
        except ValueError as e:
        ...
        if len(labels) != load_extract.shape[1]:
            if len(labels) > load_extract.shape[1]:
                ...
                labels_to_repeat = (
                    labels[-additional_needed:]
                    if additional_needed <= len(labels)
                    else labels
                )
        ...
        growth_factors = [
            ...
            for i in range(len(input_data["vehicle_category_data"]))
        ]
        ...
        for year in range(1, 6):
            ...
            ev_loads.append(
                {f"year_{year}": load_df_next.sum(axis=0).to_numpy().flatten().tolist()}
            )
        ...
        # FIXED: Required net loads calculation with proper shape handling
        required_net_loads = []
        for year in range(5):
            if year < len(selected_ranges) and year < len(ev_load_sums):
                base_load_df = pd.DataFrame(selected_ranges[year])

                # Get EV load as a 1D array (time blocks)
                ev_load_daily = ev_load_sums[year].values.flatten()  # Shape: (48,)

                # Check shapes for debugging
                logger.info(
                    f"Year {year+1}: Base load shape: {base_load_df.shape}, EV load shape: {ev_load_daily.shape}"
                )

                # Create EV load DataFrame with same shape as base load
                # Broadcast EV daily pattern across all days
                ...
                # Now both DataFrames have compatible shapes
                net_load = base_load_df + ev_load_df
                required_net_loads.append(net_load)

                logger.info(f"Year {year+1}: Net load shape: {net_load.shape}")
            else:
                # Create empty DataFrame for missing years
            ...
            for tod_item in input_data["tod"]:
                ...
                for key, value in tod_item.items():
                    if key in ["pks", "pke", "ops", "ope"]:
                        formatted_item[key] = self._format_time_for_output(value)
                    else:
        ...
        for year in range(5):
            if year < len(required_net_loads) and not required_net_loads[year].empty:
                ...
                net_load_values = required_net_loads[year].values
        ...
        values = [
            input_data["summer_peak_cost"],
            input_data["winter_peak_cost"],
            input_data["summer_op_cost"],
            input_data["winter_op_cost"],
            input_data["summer_zero_cost"],
            input_data["winter_zero_cost"],
        ]
        cost_df["utility_proc_tariff"] = np.select(conditions, values)
        final_res_x = final_res.copy()
        final_res_x.index = cost_df.index
        old_utility_cost = final_res_x.mul(cost_df["utility_proc_tariff"], axis=0) / 2
        ...
        new_utility_cost = final_res.mul(cost_df["utility_proc_tariff"], axis=0) / 2
        ...
        def calculate_tod_surcharge(year_num):
            try:
                s_pk_sum = cost_df.loc[
                    (cost_df["seasons"] == "summer") & (cost_df["unit_type"] == "pk"),
                    f"{year_num}_old_cost",
                ].sum()
                w_pk_sum = cost_df.loc[
                    (cost_df["seasons"] == "winter") & (cost_df["unit_type"] == "pk"),
                    f"{year_num}_old_cost",
                ].sum()
                s_op_sum = cost_df.loc[
                    (cost_df["seasons"] == "summer") & (cost_df["unit_type"] == "op"),
                    f"{year_num}_old_cost",
                ].sum()
                w_op_sum = cost_df.loc[
                    (cost_df["seasons"] == "winter") & (cost_df["unit_type"] == "op"),
                    f"{year_num}_old_cost",
                ].sum()
                s_0_sum = cost_df.loc[
                ...
                cost_diff = (
                    cost_df[f"{year_num}_new_cost"].sum()
                ...
                numerator = (
                    60
                    / input_data["resolution"]
                    * (old_tariff_sum + cost_diff)
                    * shared_saving
                ) / retail_tariff_df.iloc[0, 0]
